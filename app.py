...
# Pastikan kolom penting ada & tidak kosong
...
# Kolom poster opsional
if 'poster' not in df.columns:
    df['poster'] = ''

# Buat kolom gabungan untuk TF-IDF
...
def recommend(title_input, selected_genre=None, cosine_sim=cosine_sim):
    ...
    top_k = sorted_scores[1:]  # skip diri sendiri

    drama_indices = [i[0] for i in top_k]
    rec_df = df.iloc[drama_indices]

    # Filter genre jika dipilih
